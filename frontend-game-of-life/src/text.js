const about = [
  ...
  },
  {
    title: "",
    content:
      "in middleware, before the server endpoint is evoked, the image is fetched from the s3 bucket and its compatibility with sharp and background-remove is checked. \
      if it is compatible, then, the entry is placed into the specified location of the specified board. the updated board, occupied locations and palette are saved to supabase. \
      once the occupied board is completely filled, the ready code is toggled, and the game of life starts running. a cache is used to manage concurrent responses.",
  },
  {
    title: "",
    content:
      "several image processing techniques run in the background. a python script removes an image's background using a machine learning model. then the image is blurred, its saturation is increased, \
      and gets converted to raw pixel data. using a k means clustering algorithm with naive sharding, i calculated the five most common colors in the submitted image, save it, \
      and match all pixels to their closest color in lab color space. the image is then converted into png and reuploaded onto the amazon s3 bucket.",
  },
  {
    title: "",
    content:
...
const aboutImages = [
  ...
  {
    img: "/2.png",
    caption: "the first instance of kmeans image processing",
  },
  {
    img: "/3.png",
    caption: "the first instance of a rectangular game of life",
  },
  {
    img: "/4.png",
    caption: "first instance of other geometries",
  },
  {
    img: "/5.png",
    caption:
      "one of the last working images produced during the creation process",
  },
];
